<template>
  <ion-content>
                  ...
                  <ion-card-content style="margin: -15px;">
                    <form @submit.prevent="searchData">
                      <ion-item>
                        ...
                        <ion-input v-model="searchTitle" @ionInput="searchTitle = $event.target.value;" clearInput></ion-input>
                    </ion-item>
                    <ion-item>
    ...
    <ion-button expand="block" id="openCreate" style="color: #ffffff;" @click="openCreate">Új esemény létrehozása</ion-button>
    </ion-col>
    <ion-col style="margin-top: -40px; border-radius: 50px; height: auto;">
      <ion-card style="border-radius: 20px; margin-top: 55px; background-color:#ebe2d9;">
        <div v-if="filteredData.length > 0" style="margin-top: 50px; background-color:#ebe2d9;">
          <ion-row style=" margin-top: -50px;">
            <ion-col size="12" size-md="6" size-lg="3" v-for="row in filteredData" :key="row.id" style="width: 250px; ">
              <ion-card style="height: 320px; background-color: #f2eeeb; border-radius: 10px;" @click="openCard(row); openParticipantsCard()" id="openEvent">
                <img alt="Event image" :src="row.eventPictureURL || 'https://ionicframework.com/docs/img/demos/card-media.png'" style="width: 100%; max-height: 45%;"/>
                <ion-card-content>
...
<ion-modal ref="modal" class="floating-modal" trigger="openEvent" @click="closeCreate" v-if="selectedRow" :visible="modalVisible" :ionBackdropDismiss="true">
  <ion-header>
    <ion-toolbar>
  ...
  <ion-content class="ion-padding">
      ...
      <ion-card-section>
        <div v-if="selectedRow">
          <div v-if="'eventPictureURL' in selectedRow">
            <img :src="selectedRow.eventPictureURL || 'https://ionicframework.com/docs/img/demos/card-media.png'" alt="Esemény kép" class="modal-img"/>
          </div>
    ...
    <ion-card v-model="getParticipantsByEventId" style="height: auto; border-radius: 10px; border: 2px solid #186049;" class="border-td modal-card">
      <ion-header>
  ...
  <ion-footer>
    <ion-toolbar class="footer-toolbar">
        ...
        <ion-button  expand="fill" fill="solid" color="danger" style="color:black;" @click="closeCard()">Bezárás</ion-button>
      </ion-buttons>
        <ion-buttons slot="end">
        <ion-button slot="end" expand="fill" fill="solid" color="success" @click="joinEvent()" :disabled="pairExists">Csatlakozás</ion-button>
      </ion-buttons>
...
<ion-modal class="floating-modal" style="height: auto; margin-top: 0px; background-color: transparent;" v-if="createmodalVisible">
    <div class="ion-justify-content-center ion-align-items-center">
          <ion-card style="border-radius: 15px; height: auto; ">
            ...
            <ion-button color="danger" @click="closeCreate">Bezárás</ion-button>
            </ion-card-content>
          </ion-card>
        </div>
  </ion-modal>
...
.floating-modal {
  ...
  z-index: 9999; /* Ensure modal appears above other elements */
  height: 60%;
  border-radius: 16px;
